...
from config import MAX_SPRITES, MAX_CHARS_DISCORD, MAX_HISTORIAL
...
def _descargar_sprite(nombre: str) -> discord.File | None:
    ...
    try:
        ...
        return discord.File(BytesIO(response.content), filename=f"{nombre}.png")
...
async def oak_command(interaction: discord.Interaction, pregunta: str):
    ...
    if pregunta.strip().lower() == "limpiar historial":
        historial_conversaciones[usuario_id] = []
        await interaction.followup.send("✅ Historial limpiado.")
        return
    ...
    # Actualizar historial solo si todo salió bien
